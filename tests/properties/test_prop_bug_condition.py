# -*- coding: utf-8 -*-
"""
Property-based tests cho Bug Condition - Process State Lost When Session Resets.
...
from config.settings import HEARTBEAT_TIMEOUT_THRESHOLD, STATUS_DIR
from config.vn_market_rules import VN30_SYMBOLS
from models.task_models import TaskState, TaskStatus, TaskType
from orchestrator.status_protocol import write_status, read_status, read_all_statuses
...
class TestBugConditionProcessState:
    ...
    def test_new_orchestrator_detects_running_task_from_disk(self, task_status):
        """
    ...
    def test_multiple_running_tasks_detected_after_session_reset(
        self, task_status_1, task_status_2
    ):
        """
...
class TestBugConditionSettingsPersistence:
    ...
    @settings(max_examples=20, deadline=None)
    def test_preferences_persist_to_disk_and_survive_session_reset(self, preferences):
        """
    ...
    @given(preferences=preferences_strategy())
    @settings(max_examples=20, deadline=None)
    def test_init_session_state_hydrates_from_disk_file(self, preferences):
        """
        ...
        **Validates: Requirements 1.5, 1.6**
        """
        # BUG CONDITION: config/preferences.py không tồn tại → ImportError
        from config.preferences import save_preferences_to_file, load_preferences_from_file
